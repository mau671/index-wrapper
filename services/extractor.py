...
def get_password_from_database(hash_md5: str) -> Optional[str]:
    ...
    if DB_TYPE == "Supabase" and client:
        data = client.table("rar_files").select("password").eq("hash", hash_md5).execute()
...
def save_password_to_database(filename: str, hash_md5: str, password: str) -> None:
    ...
    """
    if DB_TYPE == "Supabase" and client:
        client.table("rar_files").insert({"filename": filename, "hash": hash_md5, "password": password}).execute()
...
def obtain_password(file_path: str) -> Optional[str]:
    ...
    """
    passwords = [
        "(duerumonstasu!)", "(H4mtar0!)", "(TeamKurosaki)", "by DarthMaster", "by_GfS",
        "ExcAlib444h!!", "https://www.teamkurosaki.net/", "M1rum0!!", "TeamKurosaki",
        "teamkurosaki.net", "TeamKurosaki-real89mx2", "TeamKurosaki-Rolando96",
        "TeamKurosaki-Shingeki", "www.mexanime.info", "www.teamkurosaki.net",
        "www.lascaricaturas.com", "Math", "80stvseries"
    ]

    for password in passwords:
        try:
            patoolib.test_archive(
                file_path, verbosity=-1, program="unrar", interactive=False, password=password
            )
            return password
